#!/usr/bin/perl -w
# Copyright © 2007-2024 Jamie Zawinski <jwz@jwz.org>
#
...
# implied warranty.
#
# Given a YouTube, Vimeo, Instagram or Tumblr video URL, downloads the
# corresponding MP4 file.  The name of the file will be derived from the title
# of the video.
#
...
# on how to do that.
#
# If you are wondering "why should I use this instead of "youtube-dl / yt-dlp",
# I don't really have an answer for you.  Those packages, and this one, are of
# approximately the same age.  The other ones support more sites than this one
# does, but this one is mine, and I am used to how it works, so I endeavor to
# keep it functional.  Maybe you also will like it more for whatever reason.
# With the ebb and flow of time and DRM countermeasures, one might sometimes
# work better than the other.  Use what works for you!
#
# Also, fuck Google.  Fuck them entirely.
#
#
...
##############################################################################
#
# Before you read any further, you should know what you're getting into here.
# This is a path of madness that you walk.  Below you will find two different
# custom-coded URL-loading network layers, one using a single socket with
# keep-alive and the other using a gang of pseudo-threads for a parallel
# assault on the target server, and yes, we need both; Perl code that parses
# and reconstructs minimized and intentionally obfuscated JavaScript code, and
# no I'm not joking; self-modifying code, still not joking; ciphers cosplaying
# as cryptography; three (or maybe four) different ways of parsing a list of
# available video formats out of HTML pages and out of several other terrible,
# undocumented, reverse-engineered endpoints; ever-flowing countermeasures to
# work around age gates; heuristics for deciding which of possibly hundreds of
# audio and video formats are the "best" ones; heuristics for translating
# terribly-written video titles into something more sensible to use as a file
# name; and some of the most vile and terrifying use of regular expressions
# you will ever see in all your days.  And yes, it is also all in one source
# file because fuck you that's why.  I committed these terrible acts of coding
# in anger and self defense and I have no regrets.
#
# Turn back now.  Here be monsters.
#
##############################################################################
...
sub build_http_req($$$$$$$) {
  my ($host, $path, $referer, $start_byte, $max_bytes, $extra_headers,
      $post_data) = @_;
  ...
  #
  my $hdrs = (($post_data ? 'POST' : 'GET') . " $path HTTP/1.0\r\n" .
              "Host: $host\r\n" .
  ...
  if ($post_data) {
    $hdrs .= ('Content-Length: ' . length($post_data) . "\r\n" .
              "\r\n" .
              $post_data);
  } else {
    $hdrs .= "\r\n";
...
sub get_url_1($;$$$$$$$$$) {
  my ($url, $referer, $to_file, $bwlimit, $start_byte, $max_bytes,
      $append_p, $progress_p, $extra_headers, $post_data) = @_;
  ...
  if ($to_file &&
    ...
    return get_url_1_parallel ($url, $referer, $to_file, $bwlimit,
                               ...
                               $progress_p, $extra_headers, $post_data);
  }
...
sub get_url($;$$$$$$$$$) {
  my ($url, $referer, $to_file, $bwlimit, $max_bytes, 
      $append_p, $progress_p, $force_ranges_p, $extra_headers,
      $post_data) = @_;
  ...
  do {
    ...
    my ($http, $head, $body, $bytes, $cl, $cl2) =
      get_url_1 ($url, $referer, $to_file, $bwlimit,
                 ...
                 $append_p, $progress_p, $extra_headers, $post_data);
...
sub post_url($$;$$$$$$$$) {
  my ($url, $post_data, $referer, $to_file, $bwlimit, $max_bytes, 
      $append_p, $progress_p, $force_ranges_p, $extra_headers) = @_;
  return get_url ($url, $referer, $to_file, $bwlimit, $max_bytes, 
                  $append_p, $progress_p, $force_ranges_p, $extra_headers,
                  $post_data);
}
...
#
sub qurl_make($$$$$$$) {
  my ($url, $seek_byte, $start_byte, $max_bytes,
      $bwlimit, $filename, $retry_counts) = @_;
  my ($proto, $host, $port, $path) = url_split ($url);
  my %qurl = ( 
    id            => 0,
    url           => $url,
  ...
  if ($max_bytes && $max_bytes > 0) {
    my $max_chunk_retry_count = 3;
    my $chunk = "$start_byte:$max_bytes";
    if (++$qurl{retry_counts}{$chunk} > $max_chunk_retry_count) {
      error("could not get any progress downloading chunk '$chunk' "
        . "after $qurl{retry_counts}{$chunk} attempts");
    }
  }
...
#
sub qurl_get($;$$$$$) {
  my ($qurl, $referer, $start_byte, $max_bytes, $extra_headers,
      $post_data) = @_;
...
sub qurl_read_chunk($) {
  ...
  if (!$qurl->{eof} && $qurl->{write_sock} && length($qurl->{buf})) {
    my $n = syswrite ($qurl->{write_sock}, $qurl->{buf});
    error ("file " . $qurl->{filename} . ": " . ($! || "unknown error"))
      unless defined $n;
    error ("file " . $qurl->{filename} . ": written $n bytes instead of " . length($qurl->{buf}))
      unless $n == length($qurl->{buf});
  ...
  if ($qurl->{eof}) {
    ...
    if ($qurl->{read_sock}) {
      # Disabled keepalive: it seems that the keepalive timeout will always
      # have expired by the time we get to the end of any reasonably large
      # file chunk.
      if (0 && $qurl->{keepalive} &&
...
sub get_url_1_parallel($;$$$$$$$$$) {
  ...
  my $qurl0 = qurl_make ($url, 0, $start_byte, $max_bytes,
                         $bwlimit, $to_file, undef);
  ...
  for (my $byte = $start_byte + $qurl0->{max_bytes},
    ...
    if ($size > 0) {
      my $qurl = qurl_make ($url, $seek, $byte, $size,
                            $bwlimit, $to_file, undef);
      $qurl->{id} = $i;
  ...
  while (@pending_queue || @running_queue) {
    ...
    foreach my $qurl (@running_queue) {
      if ($qurl->{eof}) {
        my $bw = $qurl->{bytes_written};
        my $size = $qurl->{max_bytes} ? $qurl->{max_bytes} - $bw : 0;
        if ($size > 0) {
          my $seek = $qurl->{seek_byte} + $bw;
          my $start = $qurl->{start_byte} + $bw;
          my $qurl1 = qurl_make($url, $seek, $start, $size, $bwlimit,
                                $to_file, $qurl->{retry_counts});
          $qurl1->{id} = $i;
          push @pending_queue, $qurl1;
          push @all_qurls, $qurl1;
          print STDERR "$progname: enqueued $qurl->{id} $start + $size = " .
                       ($start + $size) . "\n"
            if ($verbose > 3);
          $i++;
        }
...
sub video_file_size($) {
  ...
  my @cmd = ($ffmpeg,
             '-i', $file,
             '-vframes', '1', # Stop early for video files
             '-t', '1',   # Stop early for audio files
             '-codec', 'copy',
             '-f', 'null',
             '/dev/null');
...
# of Nov 2018 or so?
#
# We don't actually need to save all of the old onest that are more than
# a few months old, but I'm keeping the whole list for historical hilarity.
#
my %ciphers = (
...
# 'f55759b8/player_ias.vflset/en_US/base' => '19508 s2 w67 s2 w20 r',# 31 May 2023
# 'b128dda0/player_ias.vflset/en_US/base' => '19513 r s3 r w66 s2 w12',# 05 Jun 2023
# '8c7583ff/player_ias.vflset/en_US/base' => '19515 w47 r w26',   # 07 Jun 2023
# '0c9b5d20/player_ias.vflset/en_US/base' => '19520 r w57 s3 r s3 w20',# 12 Jun 2023
# '6ed0d907/player_ias.vflset/en_US/base' => '19522 r s1 r w56 s3 r s2',# 14 Jun 2023
# 'b7910ca8/player_ias.vflset/en_US/base' => '19529 w31 w24 r w54 r',# 21 Jun 2023
# '71547d26/player_ias.vflset/en_US/base' => '19534 r w30 w31 r s2 r s3 w52 w26',# 26 Jun 2023
# '23604418/player_ias.vflset/en_US/base' => '19536 w35 r s1 w25 r s3 w54 s1 w52',# 28 Jun 2023
# '1dda5629/player_ias.vflset/en_US/base' => '19543 w35 r s3 r',  # 05 Jul 2023
# 'f2f137c6/player_ias.vflset/en_US/base' => '19548 r w19 w22 w66 r w36',# 10 Jul 2023
# '4cc5d082/player_ias.vflset/en_US/base' => '19550 s3 r w4 r s1 r s3',# 12 Jul 2023
# '30857836/player_ias.vflset/en_US/base' => '19555 r s3 r w21 s2 w44 w1 w25 s1',# 17 Jul 2023
# '8e83803a/player_ias.vflset/en_US/base' => '19557 w42 w14 w54 s2 r w27 s3',# 19 Jul 2023
# 'd0936ad4/player_ias.vflset/en_US/base' => '19562 w4 s3 r',     # 24 Jul 2023
# '0e6aaa83/player_ias.vflset/en_US/base' => '19563 w3 r w23 w12 s3 w36 s2',# 25 Jul 2023
# '2363d0d2/player_ias.vflset/en_US/base' => '19569 w4 w1 r s1 w14 s1 r w22',# 31 Jul 2023
# 'f98908d1/player_ias.vflset/en_US/base' => '19571 s3 r w50 r w44 w26 r',# 02 Aug 2023
# '4eae42b1/player_ias.vflset/en_US/base' => '19576 w46 r s2',    # 07 Aug 2023
# '3cd2d050/player_ias.vflset/en_US/base' => '19578 w62 w62 r w18 r s1',# 09 Aug 2023
# 'da7c2a60/player_ias.vflset/en_US/base' => '19583 w9 w53 s2',   # 14 Aug 2023
# 'f980f2a9/player_ias.vflset/en_US/base' => '19585 r w37 r s1',  # 16 Aug 2023
# '21812a9c/player_ias.vflset/en_US/base' => '19590 r s2 w18 r s2 w45 r',# 21 Aug 2023
# 'c153b631/player_ias.vflset/en_US/base' => '19593 r s3 w14',    # 24 Aug 2023
# '16f9263d/player_ias.vflset/en_US/base' => '19597 s1 r s1 r w3 r s3 r s3',# 28 Aug 2023
# '0a835141/player_ias.vflset/en_US/base' => '19599 w49 w20 w25 s1',# 30 Aug 2023
# '7ee36b0e/player_ias.vflset/en_US/base' => '19605 r s1 r',      # 05 Sep 2023
# 'de91d1cf/player_ias.vflset/en_US/base' => '19611 r s1 r',      # 11 Sep 2023
# 'afd1b6e5/player_ias.vflset/en_US/base' => '19612 w27 w25 w44 s1 r w4 s2 r',# 12 Sep 2023
# '9cbb6b90/player_ias.vflset/en_US/base' => '19618 r s2 w41 s2 w64 r s3 w67 w66',# 18 Sep 2023
# '019a2dc2/player_ias.vflset/en_US/base' => '19619 r w62 s1 r s3',# 19 Sep 2023
# 'f130aa11/player_ias.vflset/en_US/base' => '19620 r w10 s1 w48 w62 s2 w25 r s1',# 20 Sep 2023
# '2626486d/player_ias.vflset/en_US/base' => '19625 w42 w19 w65 r s3',# 25 Sep 2023
# 'f7e1823a/player_ias.vflset/en_US/base' => '19627 r w41 r',     # 27 Sep 2023
  'e95f6cb1/player_ias.vflset/en_US/base' => '19632 w35 s2 w70 s1 r',# 02 Oct 2023
  '4a66ccde/player_ias.vflset/en_US/base' => '19634 r w38 r w45 r s2',# 04 Oct 2023
  '5279ebfe/player_ias.vflset/en_US/base' => '19639 r w23 s3 r s2 r w23 s3',# 09 Oct 2023
  '99faf012/player_ias.vflset/en_US/base' => '19641 r s2 w26 w12 s1',# 11 Oct 2023
  'dd34ec3d/player_ias.vflset/en_US/base' => '19646 r w54 r s2 w24 w8 s1 r',# 16 Oct 2023
  '96163992/player_ias.vflset/en_US/base' => '19654 w15 r s1 w66 w24 s2',# 24 Oct 2023
  '9e328581/player_ias.vflset/en_US/base' => '19655 w3 r s2 w26 s3 w70 s2',# 26 Oct 2023
  '0b285984/player_ias.vflset/en_US/base' => '19660 w67 s1 w31',  # 30 Oct 2023
  '9d15588c/player_ias.vflset/en_US/base' => '19662 w55 r s2 r w13 s3',# 01 Nov 2023
  '2be84af7/player_ias.vflset/en_US/base' => '19667 s2 r s1 r s2 w15 w26 s1 r',# 07 Nov 2023
  '5bdfe6d5/player_ias.vflset/en_US/base' => '19669 s2 r w50',    # 08 Nov 2023
  'f279c958/player_ias.vflset/en_US/base' => '19674 w70 s3 r s3 r s3',# 13 Nov 2023
  '190c935f/player_ias.vflset/en_US/base' => '19676 w41 s2 r w35 w24 r w6 s2',# 15 Nov 2023
  '63e90c30/player_ias.vflset/en_US/base' => '19681 r w54 r s3 w48',# 20 Nov 2023
  '5753e790/player_ias.vflset/en_US/base' => '19688 r w52 r w18 s1 r',# 27 Nov 2023
  '31e0b6d9/player_ias.vflset/en_US/base' => '19691 w30 w64 s3 w27 r',# 30 Nov 2023
  'dee96cfa/player_ias.vflset/en_US/base' => '19697 r w62 s3 r w11 w31',# 06 Dec 2023
  '0d02ada0/player_ias.vflset/en_US/base' => '19702 s2 r w48 r',  # 11 Dec 2023
  'd23221b6/player_ias.vflset/en_US/base' => '19704 w23 r s1 r w43 s3',# 13 Dec 2023
  'da154528/player_ias.vflset/en_US/base' => '19709 s3 r s2 w60 r',# 18 Dec 2023
  '4fd50162/player_ias.vflset/en_US/base' => '19725 s2 r w14 s2 w3 w55 w70 s2',# 03 Jan 2024
  '8c0b101b/player_ias.vflset/en_US/base' => '19730 r s3 w60 s3 w46',# 08 Jan 2024
  '80b90bfd/player_ias.vflset/en_US/base' => '19732 w37 w50 w64', # 10 Jan 2024
  '787e9b63/player_ias.vflset/en_US/base' => '19738 r w51 w25',   # 16 Jan 2024
  '42a553e1/player_ias.vflset/en_US/base' => '19739 w44 s1 r w58 w20 s2',# 17 Jan 2024
  'b31b88f2/player_ias.vflset/en_US/base' => '19744 w47 w10 w35 s1 w39 s2 w18',# 22 Jan 2024
  '717a6f94/player_ias.vflset/en_US/base' => '19745 w60 s1 r s3 r w5 r s3',# 23 Jan 2024
  'cb886c6c/player_ias.vflset/en_US/base' => '19746 r s2 w24 w44 s1 r w6',# 24 Jan 2024
  'f502d0a4/player_ias.vflset/en_US/base' => '19752 w39 w67 s3 r s2 r',# 30 Jan 2024
  '6ee8f9ce/player_ias.vflset/en_US/base' => '19751 s3 w16 w44 w34 w37 s3',# 29 Jan 2024
  'a1d7d0f8/player_ias.vflset/en_US/base' => '19753 w48 w13 r w7 r s1 r s1',# 31 Jan 2024
  '5e928255/player_ias.vflset/en_US/base' => '19759 s3 w9 w62 s2 w62 w10 w3',# 06 Feb 2024
  '2b39c590/player_ias.vflset/en_US/base' => '19765 r w28 r w66 r s2 w37 r',# 12 Feb 2024
  'b854ee52/player_ias.vflset/en_US/base' => '19766 w66 s3 r w42 s1 r s2 r s1',# 13 Feb 2024
  '12356a38/player_ias.vflset/en_US/base' => '19768 s1 r s2 w55', # 15 Feb 2024
  '5683fc5e/player_ias.vflset/en_US/base' => '19767 s1 w59 s1 w19 w28 s1 w15 s3 r',# 14 Feb 2024
  '2070f79f/player_ias.vflset/en_US/base' => '19773 w52 s3 w45 r w53 r s1 w29',# 20 Feb 2024
  '3ffefd71/player_ias.vflset/en_US/base' => '19774 s3 r w36 s3 r s2 w46 s1 r',# 21 Feb 2024
  '95cde7ed/player_ias.vflset/en_US/base' => '19779 r s2 w49 s3 r',# 26 Feb 2024
  '6f21feb8/player_ias.vflset/en_US/base' => '19780 r w31 s1 r s3 w56 w26 s3 w4',# 27 Feb 2024
  '31eb286a/player_ias.vflset/en_US/base' => '19781 r s1 w10',    # 28 Feb 2024
...
my $youtubei_api_key = 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8';
...
#
#                      This code is not code of honor.
#               No highly esteemed hack is commemorated here.
#               What is here is dangerous and repulsive to me.
#                 This code is best shunned and left unread.
#
sub guess_cipher($;$$) {
...
#
# Yes, that's right: this Perl program loads an HTML document, parses
# intentionally-obfuscated and minimized JavaScript from it using regular
# expressions, translates the logic in that code to another specialized
# sub-language, saves that new code into this Perl program's own source
# code, overwrites itself, and publishes an updated version of its own
# source code on the web.  The Turing Police say you have more than two
# problems.
#
sub auto_update($) {
  my ($cipher_line) = @_;
...
sub youtube_parse_urlmap($$$;$) {
  ...
  if ($urlmap =~ m/^\{"/s) {      # Ugh, sometimes it is JSON
    ...
    # Yeah I probably should have just used JSON::Any starting a decade
    # ago but I didn't and now here we are.
    # codecs=\"abc, def\"  ->  %2C
...
#
# June 2023: Aha, the trick is that only some of those "internal clients" can
# bypass the age gate, so you need to use the right key *and* clientName.
# "TVHTML5_SIMPLY_EMBEDDED_PLAYER" seems to be a good one.
...
# Loads /youtubei/ to extract the video formats.
# Populates $fmts and returns how many it parsed.
#
sub load_youtube_formats_youtubei($$$$$) {
  my ($id, $url, $sts, $cipher, $fmts) = @_;
  my $post_data = (join "\n",
                   ('{',
                    '  "context": {',
                    '   "client": {',
                    ...
                    '}'));
  $post_data =~ s/\s+//gs;
  my $info_url = ('https://www.youtube.com/youtubei/v1/player' .
                  '?key=' . $youtubei_api_key .
                  '&prettyPrint=false');
  my ($http, $head, $body) = post_url ($info_url, $post_data);
  my $err = (check_http_status ($id, $url, $http, 0) ? undef : $http);
  return 0 if $err;  # Discards HTTP error message

  my ($title) = ($body =~ m@"title":\s*"(.*?)"@s);
  ...
  foreach my $key ('formats', 'adaptiveFormats') {
    if ($body =~ m@"$key":\s*\[(.*?)\]@si) {
      my $elts = $1;
      $elts =~ s@\s*,\s*(\{\s*"itag":)@\001$1@gs;
      foreach my $f (split (/\001/, $elts)) {
        $count += youtube_parse_urlmap ($id, $f, $cipher, $fmts);
      }
    }
  }
...
#  my $retries = 5;
  ...
  if ($err && !$ping_p) {
    # If that didn't work, try the "internal player" API.
    # Don't do this with "--ping" because we want to fail
    # if it is not embeddable.
    my $count = load_youtube_formats_youtubei ($id, $url, $sts, $cipher,
                                               $fmts);
    $err = undef if ($count >= 3);
    $title = $fmts->{title} unless $title;
  }
...
sub load_youtube_formats($$$) {
  ...
  if ($ping_p && $total_retries == 0) {
    $err1 = load_youtube_formats_video_info ($id, $url, \%fmts, $ping_p);
  } elsif ($html_first_p) {
    $err1 = load_youtube_formats_html       ($id, $url, \%fmts);
    $err2 = load_youtube_formats_video_info ($id, $url, \%fmts, $ping_p);
  } else {
    $err1 = load_youtube_formats_video_info ($id, $url, \%fmts, $ping_p);
...
sub load_vimeo_formats($$$) {
  ...
  # Vimeo is now serving DASH and HLS but in a way that cannot be parsed
  # with youtube_parse_dashmpd, hooray.
  #
  if ($dash) {
    my ($http, $head, $body) = get_url ($dash);
    check_http_status ($id, $dash, $http, 1);

    my ($video) = ($body =~ m@ "video" : \[ ( .+? ) \] , [\"\}] @six);
    my ($audio) = ($body =~ m@ "audio" : \[ ( .+? ) \] \} @six);
    $video .= ",$audio" if ($audio);

    # Insert space after comma inside "segments" so we can split on },{
    $video =~ s@ ( \[ .*? \] )@{
      my $a = $1; $a =~ s/(\},)/$1 /gs; $a;
      }@gsex;

    foreach my $rep (split (/\},\{/s, $video)) {
      my ($k)    = ($rep =~ m@ "id"        : " ( .+? ) " @six);
      my ($base) = ($rep =~ m@ "base_url"  : " ( .+? ) " @six);
      my ($mime) = ($rep =~ m@ "mime_type" : " ( .+? ) " @six);
      my ($type) = ($rep =~ m@ "codecs"    : " ( .+? ) " @six);
      my ($w)    = ($rep =~ m@ "width"     : ( \d+ )     @six);
      my ($h)    = ($rep =~ m@ "height"    : ( \d+ )     @six);
      my ($segs) = ($rep =~ m@ "segments"  : \[ ( .+? ) ( \] | $ )  @six);
      print STDERR "#### $rep\n" unless $mime;
      next unless $mime; # 8-Feb-2024, WTF, have not investigated
      $type = "$mime;+codecs=\"$type\"";

      $base = url_unquote ($base);
      if ($base !~ m/^https?:/s) {
        $base = "$1/$base" if ($dash =~ m@^(https?://[^/]+)@s);
      }

      # Ok so $base looks like https://___.akamaized.net/video/___.mp4?r=___
      # and is 404.  The segments don't have URLs in them but do have
      # "range":"NNNN-NNNNN". What do I do with this shit?
      #
      my @urls = ();
      foreach my $seg (split (/\},\s*\{/, $segs)) {
        my ($url) = ($seg =~ m@ "url" : " ( .+? ) " @six);
        push @urls, $url;
      }

      #### TODO
      # print STDERR "## [$k] [$type] [$w] [$h] @urls\n";
    }
  }
...
#  308 => { v => 'mp4',  w => 2560, h => 1440, a => undef               },
  ...
  #
  # The last useful version of the table was here, 2016-04-28:
  #  https://en.wikipedia.org/w/index.php?title=YouTube&oldid=717644968
  #
  # It was "condensed" (that is, destroyed) in this edit, 2016-06-03:
  #   https://en.wikipedia.org/w/index.php?title=YouTube&oldid=723482655
  #
  # Someone tried to put some of it back, and that was deleted with the
  # comment, "All of the video formats were removed due to WP:OR
...
sub munge_title($) {
 ...
 #$title =~ s/(\s+\#[^\s]+)+$//gs;  # Remove trailing hashtags
...
sub download_video_url($$$$$$$$$$$$) {
  ...
  foreach my $target (@targets) {
    ...
    if ($size_p) {
      ...
      # Don't leave a video-only file hanging around if muxing fails.
      if (@pair && !$outfile) {
        $rm_f{$file} = 1;
      }
...
sub main() {
  ...
  # Avoid new error, "While in a single-byte locale, a multi-byte character
  # was encountered".
  my $locale = setlocale (LC_ALL) || 'C';
  setlocale (LC_ALL, "$locale.UTF-8")
    unless ($locale =~ m/UTF-8/si);
